    ...
    <div class="p-12 flex justify-center">
        <div class="max-w-[1600px] grow">
            <div class="flex justify-between items-center my-8">
                <div>
                    <h1 class="text-5xl font-semibold">User Management</h1>
                    <p class="text-muted-color mt-2">Manage console users, administrator statuses, and granular access permissions.</p>
                </div>
            ...
            <!-- Toolbar & Filter -->
            <div class="flex justify-between items-center w-full mb-6 p-4 bg-surface-0 dark:bg-surface-900 border border-surface rounded-xl">
            ...
            <div class="flex gap-3 items-center p-4 text-muted-color font-semibold border-b border-surface">
                ...
                <div class="grow-1">Name / Email</div>
                ...
                <template #list="{ items }">
                    <div class="mb-3">
                        <div v-for="user in items"
                             class="flex gap-3 items-center p-4 hover:bg-emphasis border-b border-surface"
                             :key="user.id">
                            <!-- Avatar placeholder -->
                            <div class="basis-[50px]">
                                <div class="w-10 h-10 rounded-full bg-primary-100 dark:bg-primary-950/40 text-primary flex items-center justify-center font-bold text-lg border border-primary-200 dark:border-primary-900/60">
                            ...
                            <!-- Permissions -->
                            <div class="grow-2 min-w-[300px] flex flex-wrap gap-1">
                                <template v-if="user.permissions && user.permissions.length">
                                    <Tag v-for="perm in user.permissions"
                                         :key="perm"
                                         :value="perm"
                                         severity="info"
                                         variant="outlined"
                ...
                <div class="flex flex-col gap-6 py-2">
                    ...
                    <div class="flex flex-col gap-2">
                        <label for="password" class="font-semibold">
                            Password
                            <span v-if="isEditMode" class="text-xs font-normal text-muted-color">(Leave empty to keep current password)</span>
                        </label>
                    ...
                    <div class="flex items-center justify-between p-4 bg-surface-50 dark:bg-surface-950/40 border border-surface rounded-xl">
                        <div class="flex flex-col gap-1">
                            <span class="font-semibold">Administrator Status</span>
                            <span class="text-sm text-muted-color">Grant full admin privileges and bypass granular permissions.</span>
                        </div>
                        <ToggleSwitch v-model="form.is_admin" />
                    </div>

                    <!-- Granular Permissions Selection -->
                    <div v-if="!form.is_admin" class="flex flex-col gap-3">
